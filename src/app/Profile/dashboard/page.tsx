...
import { Activity, Megaphone, MapPin, User, Calendar, Bookmark, History } from 'lucide-react';
import { db } from '@/db'; 
import { event, pemberitahuan } from '@/db/schema'; 
...
const activeEvents = await db.select()
  ...
  const pemberitahuanData = await db.select()
    .from(pemberitahuan)
    .orderBy(desc(pemberitahuan.createdAt))
    .limit(2);
