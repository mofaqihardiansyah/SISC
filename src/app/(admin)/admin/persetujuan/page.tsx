...
export default function PersetujuanEventPage() {
  ...
  return (
    <div className="space-y-10 pb-10">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard label="Total Menunggu Persetujuan" value={pendingCount} icon={FileCheck} color="blue" />
        <StatCard label="Event Disetujui" value={approvedCount} icon={CheckCircle} color="yellow" />
        <StatCard label="Total Penyelenggara Aktif" value={totalOrganizers} icon={Users} color="purple" />
      </section>
      ...
      <section className="bg-white rounded-[24px] shadow-sm border border-gray-50 p-8">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-16">
            <span className="w-6 h-6 border-2 border-slate-300 border-t-slate-900 rounded-full animate-spin" />
          </div>
        ) : events.length === 0 ? (
