...
import { updatePaperStatus } from "./actions";
...
interface ReviewPaperClientProps {
  initialPapers: PaperData[];
  initialEvents: EventData[];
}

export default function ReviewPaperClient({ initialPapers, initialEvents }: ReviewPaperClientProps) {
  const [papers, setPapers] = useState<PaperData[]>(initialPapers);
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="w-full h-[42px] pl-11 pr-4 py-2.5 border border-slate-200 rounded-xl outline-none text-sm focus:border-blue-400"
            />
          </div>
          <Select
            value={eventFilter === "all" ? "all" : eventFilter}
            onChange={(e) => setEventFilter(e.target.value === "all" ? "all" : Number(e.target.value))}
            className="h-[42px] px-4 py-2.5 border border-slate-200 rounded-xl outline-none text-sm bg-white text-slate-600 cursor-pointer min-w-44"
          >
          ...
          <Select
            ...
            className="h-[42px] px-4 py-2.5 border border-slate-200 rounded-xl outline-none text-sm bg-white text-slate-600 cursor-pointer min-w-40"
          >
